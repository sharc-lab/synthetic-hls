...
class Saver:
    ...
    # Use this path for model checkpoints.
    def get_model_dir(self) -> str:
        if self.model_logdir is None:
            # Fallback for safety if called before new_sub_saver().
            self.model_logdir = join(self.run_dir, "model")
            create_dir_if_not_exists(self.model_logdir)
        return self.model_logdir
    ...
    def save_model_state_dict(self, model: torch.nn.Module, name: str = "train_model_state_dict.pth"):
        # save checkpoints into run1/model/
        save(model.state_dict(), join(self.get_model_dir(), name))
