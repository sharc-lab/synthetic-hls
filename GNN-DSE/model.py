...
class Net(nn.Module):
    def __init__(self, in_channels, edge_dim = 0, init_pragma_dict = None, task = FLAGS.task, num_layers = FLAGS.num_layers, D = FLAGS.D, target = FLAGS.target):
        ...
        if task != 'regression':
            raise ValueError(f"[Net] This simplified model.py enforces task='regression'. Got task={task}")
        self.loss_function = nn.MSELoss()
        self.norm_method = getattr(FLAGS, "norm_method", "off")
        conv_class = GCNConv

        self.MLP_version = 'multi_obj'  if len(FLAGS.target) > 1 else  'single_obj'

        # GCNConv does not use edge_attr in this codepath (keep consistent with original GCN mode).
    ...
    def forward(self, data):
        ...
        for target_name in self.target_list:
            ...
            if self.norm_method == "logmse":
                eps = float(getattr(FLAGS, "epsilon", 1e-12))
                mse = self.loss_function(out, target)
                loss = torch.log(mse + eps)
            else:
