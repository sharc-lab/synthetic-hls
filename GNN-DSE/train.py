...
def _report_rmse_etc(points_dict, label, print_result=True):
    ...
    eps = float(getattr(FLAGS, "epsilon", 1e-12))
    tot_mape, tot_rmse, tot_mse, tot_logmse, tot_mae, tot_max_err, tot_tau, tot_std = \
...
def gen_dataset(li):
    train_loader = DataLoader(li[0], batch_size=FLAGS.batch_size, shuffle=True, pin_memory=True, num_workers=4)
...
def inference(dataset, init_pragma_dict=None, model_path=FLAGS.model_path, val_ratio=FLAGS.val_ratio, test_ratio=FLAGS.val_ratio, resample=-1, model_id=0, is_train_set=False, is_val_set=False):
    ...
    test_loss, loss_dict, gae_loss, MSE_loss = test(
        test_set, 'test', model, 0,
        plot_test=True, csv_dict=csv_dict, data_list=data_list,
        is_train_set=is_train_set, is_val_set=is_val_set
    )
...
def train(epoch, model, train_loader, optimizer, lr_scheduler, warmup_scheduler):
    ...
    for data in tqdm(train_loader):
        ...
        loss_dict, total_loss = total_loss_dict
...
def test(loader, tvt, model, epoch, plot_test = False, test_losses = [-1], csv_dict = None, data_list = [], is_train_set=False, is_val_set=False):
    ...
    sse_by_target = defaultdict(float)
    cnt_by_target = defaultdict(int)
    ...
    with torch.no_grad():
        for data in tqdm(loader):
            ...
            loss_dict, total = total_loss_dict

            for target_name in target_list:
                if 'inf' in FLAGS.subtask:
                    saver.info(f'{target_name}')
                if FLAGS.encode_log and 'perf' in target_name:
                    out = out_dict['perf']
                else:
                ...
                for i in range(len(out)):
                    ...
                    if 'inf' in FLAGS.subtask:
                        ...
                        if out_value != target_value:
                            saver.info(
                                f"{target_name} data {i} {_get_y_with_target(data, 'gname')[i]} pramga "
                                f"{_get_y_with_target(data, 'pragmas')[i][0].item()} actual value: {target_value:.2f}, "
                                f"predicted value: {out_value:.2f}"
                            )
    ...
    if 'inf' in FLAGS.subtask:
        if getattr(FLAGS, "norm_method", "off") == "logmse":
            eps = float(getattr(FLAGS, "epsilon", 1e-12))
            logmse_sum = 0.0
            for t in target_list:
                n = max(cnt_by_target[t], 1)
                mse_t = sse_by_target[t] / float(n)
                logmse_sum += float(np.log(mse_t + eps))
            inference_loss = logmse_sum
        else:
            sse = 0.0
            n_all = 0
            for t in target_list:
                for tv, pv in points_dict[t]['pred']:
                    sse += float((pv - tv) ** 2)
                    n_all += 1
    ...
    return (total / len(loader), {key: v / len(loader) for key, v in loss_dict.items()}, gae_loss, MSE_loss)