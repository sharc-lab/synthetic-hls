...
DOMAINS = {
    "sci_sim": {
        "name": "Scientific Research and Simulation",
        "focuses": [
            "Physics simulations (e.g., particle physics, high energy physics)",
    ...
    "ml_ai": {
        ...
        "focuses": [
            ...
            "Real-time Application (e.g., autonomous driving, robotics)",
        ],
    },
    "fin_model": {
        ...
        "focuses": [
            ...
            "Fraud detection and real-time credit scoring",
        ],
    },
    "eng_sim": {
        "name": "Engineering and Design Simulation",
        "focuses": [
            "Computational fluid dynamics (CFD) and aerodynamics",
            "Structural analysis and finite element analysis (FEA)",
            "Mechanical and materials engineering simulations",
            "Virtual prototyping in automotive and aerospace industries",
        ],
    },
    "data_big": {
        "name": "Data Analytics and Big Data Processing",
        "focuses": [
            "Real-time data analytics",
            "Data mining and pattern recognition",
            "Real-time processing for Internet of Things (IoT) data",
            "ETL (Extract, Transform, Load) in data warehousing",
        ],
    },
    "gfx_render": {
        "name": "Graphics Rendering and Animation",
        "focuses": [
            "3D rendering and ray tracing",
            "Real-time graphics processing in video games",
            "Computer-aided design (CAD) for complex models",
            "Visual effects and post-processing in film and media",
        ],
    },
    "crypto_bc": {
        "name": "Cryptography and Blockchain",
        "focuses": [
            "Cryptographic algorithm processing (e.g., RSA, ECC, SHA)",
            "Mining operations for cryptocurrencies",
            "Blockchain validation and consensus mechanisms",
            "Secure transaction processing in financial tech",
        ],
    },
    "telecom_sp": {
        "name": "Telecommunications and Signal Processing",
        "focuses": [
            "Signal encoding, decoding, and compression",
            "Real-time audio and video streaming",
            "Network optimization and traffic analysis",
            "5G and next-gen wireless network simulations",
        ],
    },
    "astro": {
        "name": "Astronomy and Astrophysics",
        "focuses": [
            "Cosmological simulations and large-scale universe modeling",
            "Signal processing for radio and optical telescopes",
            "Gravitational wave detection and data analysis",
            "Planetary and stellar system simulations",
        ],
    },
    "health_med": {
        "name": "Healthcare and Medical Imaging",
        "focuses": [
            "MRI, CT scan, and ultrasound image processing",
            "Real-time diagnostics and medical monitoring",
            "Genomic analysis and personalized medicine modeling",
            "Predictive models for patient care and outcomes",
        ],
    },
}
...
PROMPT_GENERAL_REQUIREMENTS = dedent(
    """
### Output and File Format Requirements:
- Do not omit any part. Do not output anything other than the required seven complete code files.
- Use the exact required filenames for the following two files:
  - `kernel_description.md`
  - `hls_eval_config.toml`
  Do not emit alternative filenames for these two files.
...
- Ensure the total design space defined by OptDSLv2 optimization template file includes a rich and balanced spread of latency vs. resource trade-offs.
...
# Prompt for seed designs generation without any input design (currently using)
...
5. Write a markdown file `kernel_description.md` that provides a concise, human-readable natural language description of the generated kernel, explaining its functionality, purpose, inputs, and outputs.
...
- The kernel must represent a coherent, end-to-end, real-world application workflow in this domain, not a toy demo, submodule or generic compute kernel.
- The core computation and intermediate representations must be domain-specific (i.e. domain-appropriate units, signals, models, features, or metrics), not just renamed generic arrays.
- The design must include at least one domain-driven correctness objective that the testbench checks (i.e. conservation/consistency constraints, error bounds, detection thresholds, scoring metrics, etc.).
- The design's end-to-end scenario, data structures, variable naming, and `kernel_description.md` must clearly reflect this domain.
- Do NOT generate a design outside this domain or switch domains mid-design.

### Originality and Practicality Constraints:
- You must create an entirely new and original design. DO NOT copy, approximate, re-implement, or repackage any pre-existing benchmarks (i.e. PolyBench, CHStone, MachSuite, or other known datasets).
- The functionality and algorithm of your design must be unique and created from scratch.
- If multiple designs are requested, each must represent a different design concept and algorithmic structure. Avoid duplicating the same kernel logic across samples.
...
{PROMPT_GENERAL_REQUIREMENTS}
"""
).strip()

# Prompt for seed designs generation with input design to optimize (NOT currently using; could ignore it fornow)
PROMPT_GEN_SINGLE_INPUT_WITH_OPT = dedent(
     f"""
You are provided with a reference HLS design containing:
...
- A `pareto_score.txt` file containing two scalar metrics that quantify the scalability of the design. Each metric is of the form:
    - `pareto_score_LUTs_vs_latency = <float>`
    - `pareto_score_FFs_vs_latency = <float>`
    These reflect how effectively the design spans a tradeoff frontier between resource usage (LUTs, FFs) and performance (latency). 
    Smaller scores indicate better scalability.

### Task Description
Your task is to:
1. Design and implement a new design as a complete high-complexity, multi-stage, application-level HLS-compatible C++ kernel with equal or better scalability (equal or smaller Pareto scores) than the reference design.
2. Write a matching C++ header file for the design.
3. Create a testbench that can validate the functionality of the design.
4. Output the name of the top-level function in a file named `top.txt`.
5. Write a markdown file `kernel_description.md` that provides a concise, human-readable natural language description of the generated design, explaining its functionality, purpose, inputs, and outputs.
6. Generate an OptDSLv2 optimization template file named `opt_template.tcl` to enable design space exploration for the kernel implementation file only. The testbench file is not relevant for optimization and should not be considered.
7. Write a toml file named `hls_eval_config.toml` whose entire contents is exactly: `tags = ["llm_gen"]`.
...
Scalability is quantitatively evaluated using Pareto scores, which measure how effectively a HLS design spans the tradeoff space between performance (latency) and hardware resource usage LUTs or FFs). 
...
Goal: Your generated design should aim to match or surpass the scalability of the reference design, achieving comparable or smaller Pareto scores on both LUTs vs. latency and FFs vs. latency.

#### Guidance for HLS Design:
Use the provided reference design and its associated scores as a guide:
- Your generated design should aim to be longer, more comprehensive, and structurally richer than the reference design wherever possible.
- Study which structures, memory organizations, and loop hierarchies lead to lower Pareto scores.
- Design your new design to match or surpass the scalability and architectural diversity of the reference, aiming for comparable or smaller Pareto scores on both LUTs vs. latency and FFs vs. latency.
- Analyze patterns in the reference `opt_template.tcl` files and design stage breakdowns to guide your own design decomposition.

### Important Constraints for the HLS Design:
- You must create an entirely new and original design. DO NOT copy, approximate, re-implement, or repackage any given or pre-existing benchmarks (i.e. PolyBench, CHStone, MachSuite, or other known datasets).
- The functionality and algorithm of your design must be unique and created from scratch.
- If multiple designs are requested, each must represent a different design concept and algorithmic structure. Avoid duplicating the same kernel logic across samples.
- The total size of the design must be moderate and practical for synthesis. Avoid extremely large loop bounds, excessive buffer sizes, or deeply nested control that could cause long compile times or unrealistic synthesis results. Typically, loop bounds should be smaller than 128.

### Structural Requirements for the HLS Design:
- You must generate an entirely new and original design that reflects a complete, multi-stage application accelerator, not a reusable or toy kernel.
- The design should represent a complete real-world application case, such as audio processing, image processing, robotics, digital signal processing, or other domains involving structured data, control logic, and computational diversity.
...
- Designs must include both compute-heavy and control-intensive stages with static loop bounds. Avoid trivial operations, isolated filters, or deeply nested control with impractical loop sizes.
...
# Prompt for fixing failed designs
PROMPT_GEN_FIX_WITH_OPT = dedent(
    f"""
You are provided with a previously generated HLS design that FAILED during evaluation.
The failure may be from C++ compilation, C++ execution (runtime/segfault), Vitis HLS synthesis, synthesis timeout or other issues.

Your task is to:
1. Diagnose the failure using the Error Information below.
2. Fix the design so it passes the evaluation end-to-end.
3. Regenerate all files with the corrections applied:
...
4. Ensure the design remains fully synthesizable by Vitis HLS.

### Error Information:
$error_message

### Critical Constraints:
- You MUST keep the design semantically consistent (same high-level functionality), unless the error requires a small structural refactor.
- The kernel + testbench must compile and run with a standard C++ compiler.
- The kernel must be synthesizable by Vitis HLS.
- Ensure `.h`, kernel `.cpp`, and testbench agree on function signatures and data types.
...
# Prompt for mutating designs to improve structural complexity metrics
...
### Task Description
Your task is to:
1. Meaningfully redesign and improve kernel complexity (structure, content, logic) along this single target dimension:
$complexity_target
...
### Complexity and Non-Regression Rules
- Improve `$complexity_target` meaningfully. Do not re-output the same design or make trivial edits.
- Cross-metric non-regression: do not reduce other `call_graph.json` metrics from baseline; increasing them is preferred.
- Treat `average_function_lines` as a direct complexity KPI: keep or improve it via real computation/control (no padding).
- Each newly added function should have LOC >= baseline `average_function_lines`.
- No padding / no gaming:
  - No-op ban: no dummy arithmetic/logical no-ops or dead stores/reads added only to raise LOC.
  - Wrapper ban: no pass-through functions whose sole purpose is calling another function without substantive in-between logic.
  - Meaningful structure only: added loops/branches must change real dataflow/control/reuse and be reflected by behavior and/or at least one OptDSL configuration’s HLS metrics.
- Non-triviality gate (each new/split function must satisfy at least one):
  - Implements a distinct algorithmic sub-stage with static bounds, or
  - Contains substantive control/dataflow that affects results, or
  - Orchestrates ≥2 non-trivial subcalls and performs meaningful in-between logic (buffering/indexing/combining).
...
# Prompt for mutating designs to improve Pareto scores
PROMPT_GEN_SCORE_FEEDBACK_WITH_OPT = dedent(
f"""
You are provided with:
- A previously generated HLS design, including kernel (.cpp), header (.h), testbench, top function name (top.txt), markdown description, and an OptDSLv2 optimization file (opt_template.tcl).
...
### Task Description
#### Goal:
Redesign and regenerate the design so that BOTH Pareto scores in `pareto_scores_summary.json` become numerically LOWER than before (e.g., if 0.52, target < 0.52; ideally ≤ 0.35), while keeping the design fully synthesizable in Vitis HLS.
...
### Complexity and Non-Regression Rules
- Cross-metric non-regression: do not regress `average_function_lines` (primary KPI), and avoid regressions in `num_functions` and `max_call_chain_depth` unless clearly justified by improved scalability.
- Non-triviality requirement: the updated design must include both compute-heavy operations and non-trivial data dependencies, enabling rich exploration under directive tuning.
- Pipeline/phases requirement: the kernel must process structured data, contain multiple computation layers or phases, and remain a self-contained functional pipeline.
- Interface and verification integrity:
